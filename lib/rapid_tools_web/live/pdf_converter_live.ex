defmodule RapidToolsWeb.PdfConverterLive do
  ...
  defp convert_uploads(socket, %{"mode" => "images_to_pdf"}) do
    ...
    case PdfConverter.images_to_pdf(consumed, output_dir: Path.dirname(hd(consumed))) do
      {:ok, result} ->
        ...
        |> assign(:batch_download_path, nil)
        |> put_flash(:info, gettext("PDF gerado com sucesso."))

      {:error, _reason} ->
        put_flash(socket, :error, gettext("Os arquivos nao puderam ser convertidos em PDF."))
    end
  end
  ...
  defp convert_uploads(socket, %{"mode" => mode}) do
    ...
    case successful_pdf_results(results) do
      ...
      :error ->
        put_flash(socket, :error, gettext("O PDF nao pode ser convertido."))
    end
  end
  ...
  defp upload_status_message(entries) do
    cond do
      entries == [] ->
        gettext("Selecione um PDF para extrair paginas ou imagens para montar um novo documento.")

      upload_in_progress?(entries) ->
        gettext("Enviando arquivos para o servidor. Aguarde todos chegarem a 100%.")

      true ->
        gettext("Uploads concluidos. Agora voce pode converter ou combinar os arquivos.")
    end
  end
              ...
              <div class="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
                  ...
                  >
                    <div class="rounded-[1.75rem] border border-dashed border-lime-200 bg-lime-50/60 p-5">
                      <div class="space-y-2">
                        ...
                        />
                        <p class="text-sm text-slate-500">
                          {gettext("Entradas aceitas: PDF, JPG, JPEG, PNG e WEBP.")}
                        </p>
                      </div>
                ...
                <aside class="rounded-[2rem] border border-white/70 bg-slate-950 p-6 text-white shadow-[0_24px_60px_rgba(15,23,42,0.16)]">
                  ...
                  <div :if={@results == []} class="space-y-4">
                    ...
                    <div class="rounded-[1.5rem] border border-white/10 bg-white/5 p-4">
                      <p class="text-sm font-semibold text-white">{gettext("Images to PDF")}</p>
