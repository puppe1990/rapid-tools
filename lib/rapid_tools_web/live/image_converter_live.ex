defmodule RapidToolsWeb.ImageConverterLive do
  ...
  def mount(_params, _session, socket) do
    {:ok,
     ...
     |> assign(:tools, [
       %{name: "Image Converter", blurb: "Batch image conversion", current: true}
     ])
     |> assign(:target_format, "png")
     |> assign(:results, [])
  ...
  def handle_event("validate", params, socket) do
    target_format = get_in(params, ["conversion", "target_format"]) || socket.assigns.target_format
    {:noreply, assign(socket, :target_format, target_format)}
  end
        ...
        if successful_results != [] and length(successful_results) == length(converted) do
          ...
          case ZipArchive.build(batch_id, batch_entries) do
            ...
            {:error, _reason} ->
              ...
              |> assign(:results, successful_results)
              |> assign(:batch_download_path, nil)
              |> put_flash(:error, "Files were converted, but the ZIP package could not be created.")
          end
        else
      ...
      <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div class="grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
          <aside class="rounded-[2rem] border border-base-300/80 bg-base-100/85 p-5 shadow-[0_18px_50px_rgba(24,24,27,0.08)] backdrop-blur">
            <div class="space-y-6">
              <div class="space-y-2">
                <p class="text-sm font-semibold uppercase tracking-[0.3em] text-primary">Rapid Tools</p>
                <div>
              ...
              <nav class="space-y-3" aria-label="Tools">
                <a
                  ...
                  class={[
                    ...
                    !tool.current && "border-base-300 bg-base-100 hover:border-primary/20 hover:bg-base-200/70"
                  ]}
                >
          ...
          <div class="space-y-6">
            ...
            <div id="converter-panel" class="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
              <div class="rounded-[2rem] border border-base-300 bg-base-100 p-6 shadow-xl">
                <.form
                  for={%{}}
                  id="converter-form"
                  phx-change="validate"
                ...
                >
                  ...
                  <div class="space-y-2">
                    <label for="target_format" class="text-sm font-semibold">Target format</label>
                    <select
                      id="target_format"
                      name="conversion[target_format]"
                      class="select select-bordered w-full"
                    >
                      <option
                        :for={format <- @formats}
                        value={format}
                        selected={format == @target_format}
                      >
                  ...
                  <button type="submit" class="btn btn-primary w-full">Convert image</button>
                </.form>
              </div>
              ...
              <aside class="rounded-[2rem] border border-base-300 bg-base-100 p-6 shadow-xl">
                <div :if={@results != []} class="space-y-4">
                  ...
                  <a :if={@batch_download_path} href={@batch_download_path} class="btn btn-accent w-full">
                    Download all as ZIP
                  </a>
                  <div class="space-y-3">
                    <div
